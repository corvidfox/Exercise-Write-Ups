...
```{r}
rusXs <- rep(whiskey$Distillery, each=dim(whiskey)[1])
rusYs <- rep(whiskey$Distillery,dim(whiskey)[1])

ruData<-data.frame(rusXs,rusYs)
...
for(i in seq(dim(distilleryCorrelations)[1])){
  ...
  for(j in seq(dim(distilleryCorrelations)[1])){
    ...
    if (distilleryCorrelations[i,j] >= 0.7){
      ...
      colWhiskeyRegion <- whiskey[whiskey$Distillery == colnames(distilleryCorrelations)[i],]$Region
      rowWhiskeyRegion <- whiskey[whiskey$Distillery == colnames(distilleryCorrelations)[j],]$Region
...
ruData$Correlations<-ruCorr
ruData$Colors<-ruColors
colnames(ruData) <- c("RU-Ys", "RU-Xs","Correlation","Region Color")
```

And finally, we build our rBokeh visualization. Since there are so many values for distillery, we hide our x and y axes labels for readability. We ensure those values are in the `hover` information.

```{r}
rurBokeh <- figure(title="RUrBokeh") %>%
  ly_crect(rusXs, rusYs, data = ruData, 0.9, 0.9,
           fill_color = ruColors, line_color = ruColors, hover = "Whiskies: @rusXs,@rusYs. Correlation: @ruCorr") %>%
