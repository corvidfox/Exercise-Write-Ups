...
for (i in seq(dim(grid)[1])){
  bokehColors <- c(test,colors[(i%%2)+1])
}
```
...
basicBokeh <- figure(title="Basic rBokeh", tools = "hover") %>%
  ly_crect(xs, ys, data = grid, 0.9, 0.9,
           fill_color = bokehColors, line_color = bokehColors, fill_alpha = alphas, line_alpha = alphas, hover = "Values: @xs,@ys", xlab="", ylab="")
basicBokeh
```
